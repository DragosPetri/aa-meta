---
name: attach-setup
description: Register an analog attachable binary with analog-attach (init + config session) and guide a user through configuring a device
---

# Attach Setup

You are guiding a user through one or both of these workflows using the `analog-attach` CLI:
...
```
analog-attach schema responses
```
...
If `.analog-attach.toml` exists in the current directory, check whether it already has a registered tool and skip directly to Phase 2 for that tool. A user can also do both phases in sequence.
...
Run `analog-attach --json suggest attachable` to discover known attachable binaries on PATH. Present the results to the user and let them pick one, or enter a custom binary name/path.
...
Run:
```
analog-attach --json init <binary> --no-interactive
```
...
Read the response schema (`analog-attach schema responses`) to parse the `InitResponse`:
...
Run:
```
analog-attach --json tool-config-get
```
...
6. Once the user confirms a value, run:
   ```
   analog-attach --json tool-config-set <field_name> <value>
   ```
   If `ok: false`, show the error and re-prompt. If `ok: true`, mark the field as configured.

After all required fields are handled, re-run `analog-attach --json tool-config-get` to confirm every required field now has a non-null `value`.
...
Run `analog-attach --json tool-config-get`. If this fails or returns `ok: false`, direct the user to Phase 1 first.

Show a brief summary: tool name, configured fields and their values.

**Read the manifest for command descriptions**: Locate the registered tool's manifest path in `.analog-attach.toml`, then read it (it is a JSON file). Each `CommandMapping` may carry a `description` string written by the tool author that explains what the command does for *this specific tool*. Use these descriptions throughout Phase 2 when explaining to the user what an action will do — prefer the tool-author's wording over generic protocol descriptions. If no description is present for a command, fall back to the protocol's own definition.

### Step 1: Discover available intelligence

Run:
```
analog-attach --json list-intelligence
```
...
Run:
```
analog-attach --json list-devices
```
...
Run:
```
analog-attach --json create-workfile
```
...
Run `analog-attach --json list-devices` again to confirm the initial device list.
...
Run:
```
analog-attach --json add <key> [--name <name>] [--to <parent...>]
```
...
Run:
```
analog-attach --json read [<identifier>...]
```
...
Run:
```
analog-attach --json update <path...> --with <value>
```
...
```
analog-attach --json delete <path...>
```
...
```
analog-attach --json delete <path...> --force
```
...
Run:
```
analog-attach --json validate [<path...>]
```
...
Run `analog-attach --json tool-config-get` and show the current values.
...
- Always pass `--json` as a global flag before the subcommand: `analog-attach --json <subcommand> [args]` — this produces parseable JSON on stdout; without it, output goes to stderr in human-readable form and is harder to parse
- Always read the response schema (`analog-attach schema responses`) before interpreting a response — type descriptions contain discriminators, rendering hints, and semantic notes
